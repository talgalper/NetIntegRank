...
- (Mini)Conda (recommended) OR container runtime (Docker/Apptainer) if you set containers
...
> **Similarity matrix caching:** HHNet constructs a `similarity_matrix.h5` file from the processed PPI network, and this step can take a while for large networks. To avoid recomputing it on later runs, the wrapper now caches the similarity matrix (and associated `beta.txt`) using the canonicalised/indexed PPI network as the cache key. If the same network is used again, the cached files are reused automatically.
...
The Machine Learning (ML) scores were obtained by running a random forest model on a large set of features extarcted from a previous study as well as some custom features input by me. The paper the model was based off can be found here: [https://pubmed.ncbi.nlm.nih.gov/32171238/](https://pubmed.ncbi.nlm.nih.gov/32171238/)
...
If `--gene_map` is not provided, `run_ranking.R` will attempt to reuse an annotation cache before falling back to biomaRt.

By default, the cache path is:

* `${outdir}/ranking/id_annot_cache.tsv`

If that cache file already exists, it will be reused and updated only for missing IDs.
Users can also provide an explicit cache path with:

* `--id_annot_cache path/to/id_annot_cache.tsv`

This can be useful if you want to reuse the same cache across runs with different output directories.
...
* `--gene_map` (default: null)

  * Optional mapping table used instead of biomaRt lookups
* `--id_annot_cache` (default: `${outdir}/ranking/id_annot_cache.tsv`)

  * Optional explicit cache path for Ensembl → UniProt / gene symbol annotations
  * If the cache already exists, it is reused and updated for any missing IDs
...
* If `--gene_map` is not provided, biomaRt may be used for missing annotations and therefore requires internet access.
* To reduce or avoid repeated biomaRt calls, provide either `--gene_map` or a reusable `--id_annot_cache` file.
* By default, the ranking annotation cache is written to `${outdir}/ranking/id_annot_cache.tsv`.
* Similarity matrices generated by HHNet are cached under the HHNet directory in `cache/similarity_matrices/`. Reusing the same processed PPI network should therefore speed up later runs. The cache directory must be writable.